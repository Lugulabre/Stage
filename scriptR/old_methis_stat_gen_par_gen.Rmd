...
## Fonction de lecture d'une stat
...
data.frame.stat = function(seq_ne, max_gen){
  ...
  for (ne in seq_ne) {
    ...
    mat_stat[row_min:row_max,1] = str_c("Ne", toString(ne)) #Ecriture du Ne correspondant pour les stats qui seront écrites
...
plot_stat_gen = function(df, gen, stat, Ne){
  p = ggplot(df, aes(x = gen, y = stat, color = Ne)) +
...
seq_tot = c(seq_60_100, seq_64_8192)

mat_tot = data.frame.stat(seq_tot, 100)
mat_60_100 = data.frame.stat(seq_60_100, 100)
mat_64_8192 = data.frame.stat(seq_64_8192, 100)
...
mat_tot = data.frame.stat(seq_tot, 100)
mat_end_900 = data.frame.stat(seq_end_900, 100)
mat_start_30 = data.frame.stat(seq_start_30, 100)
