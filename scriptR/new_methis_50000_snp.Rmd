...
data.frame.stat = function(seq_ne, max_gen, max_simu){
  ...
  for (ne in seq_ne) {
    if (dir.exists(str_c("Ne", toString(ne), "/"))) {
      ...
      for (nb_simu in 1:max_simu) {
        ...
        # delta0 = mat_stat[row_min_simu,32]
        # print(delta0)
        # mat_stat[row_min_simu:row_max_simu,65] = (delta0 - mat_stat[row_min_simu:row_max_simu,32])
      }
      #print(dim(mat_stat))
    }
  }
  ...
  colnames(mat_stat) = c("Ne", "simu", names(file_stat))#,"cpt")#, "delta.adm.props")
...
hist.gif.props.adm = function(mat, select_stat, select_seq, title,
  ...
  for (line in seq_gen) {
    ...
    legend("topleft", title="Alpha",legend = select_seq,
...
for (nu in seq_nu) {
  ...
  p_tmp = plot_stat_gen(mat_pop_nu_tmp, mat_pop_nu_tmp$Generation,
                        mat_pop_nu_tmp$mean.het.adm, mat_pop_nu_tmp$Ne,
                        mat_pop_nu_tmp$Ne,
                        str_c("U = ",nu), TRUE, legd = FALSE)
  p_tmp = p_tmp+geom_hline(yintercept = mat_pop_nu_tmp$mean.het.s1[1],
                           color = "red")
  p_tmp = p_tmp+geom_hline(yintercept = mat_pop_nu_tmp$mean.het.s2[1],
                           color = "black")
  print(p_tmp)
...
```{r}
# mat_cstt_tmp = extract_sub_mat(mat_64_100_16384, c(64, 128, 256, 1024, 2048))
# mat_cstt_tmp$ne_gen = unlist(mat_ne_inc , use.names = F)
# 
# p = ggplot(mat_cstt_tmp,aes(x = Generation, y = ne_gen, group = Ne, color = Ne))
# p = p + geom_line(show.legend = TRUE) + ggtitle("Évolution du Ne au cours des générations")
# p = p+labs(color = "Ne")+ylab("Ne")+theme_classic()
# print(p)
```

```{r}
# 
# mat_bott_tmp = mat_bottle_time_80
# mat_bott_tmp$ne_gen = unlist(mat_ne_inc , use.names = F)
# # 
# # mat_bott_tmp$ne_gen
# p = ggplot(mat_bott_tmp,aes(x = Generation, y = ne_gen, group = U, color = U))
# p = p + geom_line(show.legend = TRUE) + ggtitle("Évolution du Ne au cours des générations")
# p = p+labs(color = "U")+ylab("Ne")+theme_classic()
# print(p)
# 
```
