...
mat_prior_tot = data.frame.prior(seq_tot, 1)
...
```{r}
seq_64_8192 = 2**seq(10,14,1)
seq_60_100 = 10*seq(5,100,1)
seq_tot = sort(c(seq_60_100, seq_64_8192))
...
Graphiques obtenus
```{r}
p = plot_stat_gen(mat_tot, mat_tot$gen, 
              mat_tot$mean.het.adm, mat_tot$Ne,
              "Stat en fonction des générations\n selon différentes Ne initiales",
              legd = FALSE)
p = p + geom_hline(yintercept = mat_tot$mean.het.s1[1],
                           color = "red")
p = p + geom_hline(yintercept = mat_tot$mean.het.s2[1],
                           color = "black")
print(p)
