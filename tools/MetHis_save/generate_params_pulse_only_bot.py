...
def parse_args() :
    ...
    parser.add_argument('--Nu',
                        ...
                        type=float,
                        dest='nu_value',
                        help='Value of growth curvature x 100')
    parser.add_argument('--alpha',
                        default=random.uniform(0,1),
                        type=float,
                        dest='alpha',
                        help='Decay value in a bottleneck')
    parser.add_argument('--tb',
                        default=2,
                        type=int,
                        dest='tb',
                        help='Time of a bottleneck for the corresponding scenario')
    parser.add_argument('--pulse_only_bot',
                        default=False,
                        dest='p_only_bot',
                        help='If True, pulses occurs only during bottleneck')
    ...
    return values.nb_simulation, values.nb_generation, values.prefix, values.Ne, values.contrib_s1, values.contrib_s2, values.nu_value, values.alpha, values.tb, values.p_only_bot, values.force_rewrite
...
def parse_Ne_str(Ne_str) :
    ...
    try :
        ...
        exit_on_Ne_str_parsing_fail(Ne_str)
    if Ne_splitted[1] not in ('Con','Inc','Dec','All','Bot') :
        exit_on_Ne_str_parsing_fail(Ne_str)
...
def make_Ne_sanity_check(Ne, alpha) :
    Ne0 = Ne[0]
    scheme = Ne[1]
    ...
    if scheme == 'Dec' and min_final > Ne0 :
        exit_on_Ne_inconsistency()
    if scheme == 'Bot' and max_final < Ne0*alpha:
        exit_on_Ne_inconsistency()
...
def generate_non_constant_pattern(x1, x2, y1, y2, increase, nu_val) :
    if x2 == x1 :
...
def generate_Nes(pattern, nb_generation, nu_val, alpha, tb) :
    Ne0 = pattern[0]
    scheme = pattern[1]
    ...
    while failed :
        ...
        if tmp_scheme == 'Inc' :
            ...
            if tmp_final < Ne0 :
                continue
            u, Nes = generate_non_constant_pattern(1, nb_generation+1, tmp_final, Ne0, True, nu_val)
            new_Nes.extend(Nes)
        ...
        if tmp_scheme == 'Dec' :
            ...
            if tmp_final > Ne0 :
                continue
            u, Nes = generate_non_constant_pattern(1, nb_generation+1, Ne0, tmp_final, False, nu_val)
            new_Nes.extend(Nes)
            new_Nes = [int(round(j)) for j in new_Nes]
            failed = False
        if tmp_scheme == 'Bot' :
            tmp_final = random.randint(final_min, final_max)
            if tmp_final < Ne0*alpha :
                continue
            u, Nes = generate_non_constant_pattern(1, tb, tmp_final, Ne0*alpha, True, nu_val)
            Nes = [float(Ne0)]+Nes+[float(tmp_final)]*(nb_generation+1-tb)
            new_Nes.extend(Nes)
            new_Nes = [int(round(j)) for j in new_Nes]
            failed = False
    return u, new_Nes
...
def generate_pulse_contribution(pattern, nb_generation, tb, p_only_bot) :
    c0 = pattern[0]
    nb_pulse = pattern[2]
    if p_only_bot :
        generations = range(1, tb + 1)
    else :
...
def generate_contribution(pattern, nb_generation, nu_val, tb=0, p_only_bot=False) :
    c0 = pattern[0]
    scheme = pattern[1]
    # we make a special case for Pulse scheme
    if scheme == 'Pulse' :
        u, new_contribs = generate_pulse_contribution(pattern, nb_generation, tb, p_only_bot)
    ...
    while failed :
        ...
        if tmp_scheme == 'Con' :
            ...
            if not p_only_bot :
                ...
                new_contribs.extend([tmp] * tb)
                new_contribs.extend([0] * (nb_generation-tb))
            u = 'NA'
            failed = False
        if tmp_scheme == 'Inc' :
            tmp_final = 1.0
            ...
            if tmp_final < tmp_init :
                continue
            if not p_only_bot :    
                u, contrib = generate_non_constant_pattern(1, nb_generation, tmp_final, tmp_init, True, nu_val)
                new_contribs.extend(contrib)
            else :
                u, contrib = generate_non_constant_pattern(1, tb, tmp_final, tmp_init, True, nu_val)
                new_contribs.extend(contrib)
                new_contribs.extend([0] * (nb_generation - tb))
            failed = False
        if tmp_scheme == 'Dec' :
            ...
            if tmp_final > tmp_init :
                continue
            if not p_only_bot :
                u, contrib = generate_non_constant_pattern(1, nb_generation, tmp_init, tmp_final, False, nu_val)
                new_contribs.extend(contrib)
            else :
                u, contrib = generate_non_constant_pattern(1, tb, tmp_init, tmp_final, False, nu_val)
                new_contribs.extend(contrib)
                new_contribs.extend([0] * (nb_generation - tb))
            failed = False
    return u, new_contribs
...
def main() :
    ...
    nb_simulation, nb_generation, prefix, Ne_str, contrib_s1_str, contrib_s2_str, nu, alpha, tb, p_only_bot, force_rewrite = parse_args()
    ...
    Ne_pattern = make_Ne_sanity_check(Ne_pattern, alpha)
    ...
    if s1_pattern[1] == 'Pulse' and p_only_bot and len(s1_pattern) > tb + 3 :
        exit_on_nb_pulse_error(1)
    if s2_pattern[1] == 'Pulse' and p_only_bot and len(s2_pattern) > tb + 3 :
        exit_on_nb_pulse_error(2)
    try :
    ...
    for i in xrange(1, nb_simulation+1) :
        uNe, new_Nes = generate_Nes(Ne_pattern, nb_generation, nu, alpha, tb)
        uNe = nu
        ...
        while not contribs_compatible :
            ...
            nb_try += 1
            if not p_only_bot:
                u1, contribs_s1 = generate_contribution(s1_pattern, nb_generation, nu)
                u2, contribs_s2 = generate_contribution(s2_pattern, nb_generation, nu)
            else :
                u1, contribs_s1 = generate_contribution(s1_pattern, nb_generation, nu, tb, p_only_bot)
                u2, contribs_s2 = generate_contribution(s2_pattern, nb_generation, nu, tb, p_only_bot)
